...
func (s *CachedStore) Resolve(ctx context.Context, reference string) (ocispec.Descriptor, error) {
	...
	if err != nil {
		if errors.Is(err, errdef.ErrNotFound) {
			...
			return contentstoreutil.ResolveBlob(os.DirFS(s.root), reference)
		}
...
func (s *CachedStore) Tags(ctx context.Context, last string, fn func(tags []string) error) error {
	...
	return contentstoreutil.ListTags(s.tagResolver, last, fn)
}
...
func (s *CachedStore) ensureOCILayoutFile() error {
	...
	return contentstoreutil.ValidateOCILayout(&layout)
}
...
func (s *CachedStore) loadIndexFile(ctx context.Context) error {
	...
	s.index = &index
	return contentstoreutil.LoadIndex(ctx, s.index, s.storage, s.tagResolver, s.graph)
}
...
func (s *CachedStore) saveIndex() error {
	...
	for ref, desc := range refMap {
		if ref == desc.Digest.String() && !tagged.Contains(desc.Digest) {
			...
			manifests = append(manifests, contentstoreutil.DeleteAnnotationRefName(desc))
		}
	}
...
func (s *CachedStore) gcIndex(ctx context.Context) error {
	...
	for ref, desc := range refMap {
		...
		if err := tagResolver.Tag(ctx, contentstoreutil.DeleteAnnotationRefName(desc), desc.Digest.String()); err != nil {
			return err
		}
